...
% This uses xgboost as an example to show how to call the xgboost algorithm
% as-is (default parameters), custom values for parameters,
% use a CV strategy and pass custom values for CV parameters 


%This is a small test of the scikit_from_matlab package using xgboost it shows how 
%for windows i put in the dll files in a subfolder. seems like that is the
%only way i could run xgboost. maybe expanding the matlab python path to
...
%it seems that both GridSearchCV and xgboost uses only 1 thread if this variable is not specified
...
%Let's try XGBoost https://xgboost.readthedocs.io/en/latest/parameter.html
...
%lets try max_depth, learning_rate and gamma for xgboost
fprintf('Tutorial 3: running %s with additional parameter of GridSearchCV and trying (max_depth, learning_rate, and gamma) search\n', algo_name);
algo_params = py.dict;
CV_strategy = 'GridSearchCV';
CV_params_for_algo = py.dict;
update(CV_params_for_algo, py.dict(pyargs('max_depth',    int32(3:2:10)     )))
update(CV_params_for_algo, py.dict(pyargs('learning_rate',double(0.1:0.2:1) )))
update(CV_params_for_algo, py.dict(pyargs('gamma',        int32([0,5])     )))
tic
...
fprintf('Tutorial 4: running %s with additional parameter of GridSearchCV and trying (max_depth, learning_rate, and gamma) search\n', algo_name);
...
update(CV_params_for_algo, py.dict(pyargs('max_depth',    int32(3:2:10)     )))
update(CV_params_for_algo, py.dict(pyargs('learning_rate',double(0.1:0.2:1) )))
update(CV_params_for_algo, py.dict(pyargs('gamma',        double([0,5])     )))
...
%now we use some form of CV, lets try GridSearchCV, with search over
%lets try max_depth, learning_rate and gamma for xgboost
fprintf('Tutorial 7: running %s with additional parameter of GridSearchCV and trying (max_depth, learning_rate, and gamma) search\n', algo_name);
algo_params = py.dict;
CV_strategy = 'GridSearchCV';
CV_params_for_algo = py.dict;
update(CV_params_for_algo, py.dict(pyargs('max_depth',    int32(3:2:10)     )))
...
%lets try max_depth, learning_rate and gamma for xgboost 
%and pass parameters to GridSearchCV
fprintf('Tutorial 8: running %s with %s with additional parameter of GridSearchCV and trying (max_depth, learning_rate, and gamma) search\n', algo_name);
fprintf('Furthermore we now select a CV time of 4 and do refit=true\n')
algo_params = py.dict;
CV_strategy = 'GridSearchCV';
CV_params_for_algo = py.dict;
